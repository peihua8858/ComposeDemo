...
public class LyricTextView extends View {
    ...
    private Paint paint;
    private float textWidth; //text的宽高
    ...
    public LyricTextView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        direction = t.getInt(R.styleable.LyricTextView_ltv_direction, LEFT);
...
//        textHeight = r.bottom - r.top;
    ...
    private int measure(int measureSpec, boolean isWidth) {
        ...
        switch (mode) {
            ...
            case MeasureSpec.UNSPECIFIED:
                if (isWidth) {
                    size = (int)  Math.ceil(textWidth);
                } else {
                    size = (int) Math.ceil(textHeight);
                }
    ...
    private void drawText(Canvas canvas, int direction, float progress) {
        ...
        float textLeft = getPaddingLeft() + realWidth / 2 - textWidth / 2;   //文本在控件中的起始x位置
        float textRight = getPaddingLeft() + realWidth / 2 + textWidth / 2;   // 文本在控件中的结束x位置
        float textBottom = getPaddingTop() + realHeight / 2 + textHeight / 2;  //文本在控件中的结束y位置
        if(progress < 0 ){progress = 0;}
        if(progress > 1 ){progress = 1;}
        float changedWidth =  (textWidth * progress);
        if (direction == LEFT) {
            startX = textLeft;
...
//        canvas.drawText(text, textLeft, textBottom, paint);
        ...
        canvas.restore();
        Logcat.d("ccy", "startX = " + startX + ";endX = " + endX);
        //画渐变部分的文字
