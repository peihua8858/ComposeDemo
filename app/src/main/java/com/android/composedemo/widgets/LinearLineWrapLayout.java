...
public class LinearLineWrapLayout extends ViewGroup {
    ...
    private void measure(int widthMeasureSpec, int heightMeasureSpec, int widthMode, int widthSize, int heightMode, int heightSize) {
        ...
        View childView;
        LinkedList<View> rowViews = new LinkedList<>();
        ...
        for (int position = 0; position < getChildCount(); position++) {
            ...
            if (widthSizeUnspecified) {
                ...
                if (rowWidth + childViewWidth > availableWidth) {
                    ...
                    adjustWidth = adjustWidth + 1;
                }

                if (mMaxShowLine != -1 && adjustWidth >= mMaxShowLine) {
                    break;
                }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        int lineNum = 0;
        int w, count;
        for (w = 0, count = getChildCount(); w < count; w++) {
            ...
            if (mMaxShowLine != -1 && lineNum >= mMaxShowLine) {
                break;
            }
            ...
            if (getLayoutDirection() == LAYOUT_DIRECTION_RTL) {
                childView.layout(viewWidth - right, top, viewWidth - left, bottom);
            } else {
        ...
        hasNextLine = w <= count - 1;
        mCurrentShowLine = lineNum;
    }
    ...
    public void setMaxShowLine(int mMaxLine) {
        this.mMaxShowLine = mMaxLine;
    }
